...
bool APP_SERCOM_I2C_Callback ( I2C_SLAVE_TRANSFER_EVENT event, uintptr_t contextHandle )
{
    ...
    switch(event)
    {
        case I2C_SLAVE_TRANSFER_EVENT_ADDR_MATCH:
            ...
            if (eepromData.internalWriteInProgress == true)
            {
        ...
        case I2C_SLAVE_TRANSFER_EVENT_RX_READY:
            ...
            {
                eepromData.wrBuffer[(eepromData.wrBufferIndex & EEPROM_PAGE_SIZE_MASK)] = I2C1_ReadByte();
        ...
        case I2C_SLAVE_TRANSFER_EVENT_STOP_BIT_RECEIVED:
            ...
            if (eepromData.wrBufferIndex > 0)
            {
                ...
                eepromData.nBytesToWrite = eepromData.wrBufferIndex;
...
void EEPROM_StateMachine(void)
{
    ...
    switch(eepromData.eepromCommand)
    {
        case EEPROM_CMD_WRITE:
            do
            {
                /* Calculate the number of bytes that can be written from current address till end of the page */
                nWrBytes = EEPROM_PAGE_SIZE_BYTES - (eepromData.currentAddrPtr % EEPROM_PAGE_SIZE_BYTES);  
                
                if (nWrBytes > eepromData.nBytesToWrite)
                {
                    nWrBytes = eepromData.nBytesToWrite;
                }
                memcpy(&EEPROM_EmulationBuffer[eepromData.currentAddrPtr], &eepromData.wrBuffer[wrBufferIndex], nWrBytes);
                
                eepromData.currentAddrPtr += nWrBytes;                
                ...
                /* Update the bytes pending to be written */
                eepromData.nBytesToWrite -= nWrBytes;
                wrBufferIndex += nWrBytes;
                
            }while(eepromData.nBytesToWrite > 0);
            
            /* Delay to simulate EEPROM internal write busy state */
            delay(10000);
